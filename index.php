...
<div class="wrap">
  <header>
    ...
    <div class="headerRight">
      <span id="ethPrice" class="pill">ETH —</span>
      <span id="xrpPrice" class="pill">XRP —</span>
      <span id="adaPrice" class="pill">ADA —</span>
      <span id="goldPrice" class="pill">Gold —</span>
      <span id="oilPrice" class="pill">Oil —</span>
    </div>
  </header>
  ...
  <div class="settingsContent">
    ...
    <div class="settingsSection">
      ...
      <div class="settingOption" style="flex-direction:column;align-items:flex-start;gap:8px">
        <span>🚫 Block words (comma separated)</span>
...
const seenTrades = new Set(); // Track processed trades to prevent duplicates
...
<div class="col" id="col-news" style="display:none">
  <div class="colHeader">
    <div class="tfLabel">📰 LIVE NEWS</div>
    <div class="statPills">
...
<div class="col" id="col-ticker" style="display:none">
  <div class="colHeader">
    <div class="tfLabel">📊 LIVE TICKER</div>
    <div class="statPills">
...
function onTrade(t){
  ...
  // CRITICAL FIX: Skip duplicate trades to prevent infinite MySQL save loop
  // restFallback() fetches last 20 trades every 1.2s, many of which were already processed
  if(seenTrades.has(time)){
    return; // Skip this trade - already processed
  }
  seenTrades.add(time);

  // Prune seenTrades Set to prevent memory growth (keep last hour of trades)
  // At ~3 trades/second, 1 hour = ~10,800 trades, but Set only stores timestamps
  if(seenTrades.size > 15000){
    const sortedTimes = Array.from(seenTrades).sort((a,b) => a - b);
    const keepFrom = sortedTimes.length - 10000;
    seenTrades.clear();
    sortedTimes.slice(keepFrom).forEach(t => seenTrades.add(t));
  }
...
function prune(){
  ...
  if(before5m > 0 && win5m.length === 0){
    console.error(`⚠️ 5m array cleared! Before: ${before5m}, After: 0. Current time: ${now}, Cutoff: ${now - 300000}`);
  }
...
function updateTopTrades(key, liveArr){
  ...
  if(key === '30d'){
    // console.log(`🔍 30d liveArr has ${liveArr.length} total trades (before filtering by side)`);
  }
  ...
  if(key === '30d'){
    // console.log(`📊 30d updateTopTrades: ${buys.length} total buys, ${sells.length} total sells`);
  ...
  if((key === '30d' || key === '24h' || key === '12h' || key === '8h' || key === '1h')){
    if(isLoadingFromMySQL){
      // if(key === '30d') console.log(`🔒 ${key}: Save blocked - still loading from MySQL`);
    } else if(!buysChanged && !sellsChanged){
      // if(key === '30d') console.log(`⏭️ ${key}: No changes, skipping save`);
    } else {
      console.log(`💾 Saving ${key}: ${top10Buys.length} buys, ${top10Sells.length} sells to MySQL`);
...
async function loadTopTradesFromMySQL(){
  try{
    ...
    console.log('✅ Loaded top trades from MySQL');
...
function mergeMySQLTradesWithLive(data){
  ...
  TF.forEach(tf => {
    ...
    if(data[key]){
      ...
      console.log(`📥 Loading ${key}: ${fbBuys.length} buys, ${fbSells.length} sells from MySQL`);
      ...
      // CRITICAL FIX: Update prevTopTrades to prevent infinite save loop
      // When loading from MySQL, we need to set prevTopTrades so the next
      // updateTopTrades() call doesn't think everything changed
      // IMPORTANT: Sort by USD value (same as updateTopTrades does) to ensure order matches
      const sortedBuys = [...fbBuys].sort((a, b) => b.usd - a.usd);
      const sortedSells = [...fbSells].sort((a, b) => b.usd - a.usd);
      prevTopTrades[key].buys = sortedBuys.map(t => t.time);
      prevTopTrades[key].sells = sortedSells.map(t => t.time);
...
async function fetchHistoricalCandles(){
  try{
    ...
    if(r30d.ok){
      ...
      console.log('✅ Loaded', candles.length, '30d candles');
    }
    ...
    isLoadingFromMySQL = false;
    console.log('🔓 Initial load complete, MySQL saves enabled');
...
async function updateWeather(){
  try {
    ...
    const emoji = weatherEmojis[condition] || (isDaytime ? '🌤️' : '🌙');
    ...
    const tomorrowEmoji = forecastEmojis[tomorrowCondition] || '🌤️';
      ...
      if(rOil.ok){
        ...
        if(dataOil && dataOil.data && dataOil.data.price){
          ...
          if(price > 0){
            ...
            if(!oilMarketOpen){
              console.log('🛢️ Initial oil price loaded (markets closed, showing last close)');
            }
          }
        }
      }
    } else if(!shouldFetchOil && !oilMarketOpen){
      console.log('🛢️ Oil markets closed - skipping API call');
    }
  ...
  try{
        ...
        fetch(`/api-rss.php?rss_url=${encodeURIComponent(feed.url)}&count=10`)
          ...
          .then(data => {
            ...
            if(data.items && data.items.length > 0){
              console.log(`\n📅 ${feed.name} - Raw date from RSS: ${data.items[0].pubDate}`);
            }
    ...
    responses.forEach(feed => {
      if(feed.items.length > 0){
        console.log(`\n📰 ${feed.name} (${feed.items.length} items):`);
    ...
    responses.forEach(feed => {
      feed.items.forEach(item => {
        ...
        if(item.title && !item.title.trim().startsWith('[No Title]')){
          ...
          if(hasBlockedWord) {
            console.log(`🚫 Filtered out: "${item.title.substring(0, 60)}..." (matched filter)`);
          ...
          if(!isNaN(parsedDate.getTime())){
            ...
            if(feed.name === 'Trump Truth'){
              console.log(`   ⚠️ Trump item FAILED date validation: "${item.title?.substring(0, 50)}" | pubDate: ${item.pubDate}`);
            }
          }
        } else {
          // Debug: Log items with no title
          if(feed.name === 'Trump Truth'){
            console.log(`   ⚠️ Trump item MISSING TITLE`);
          }
        }
      });
    });

    console.log(`\n✅ Trump Truth items added to allNews: ${trumpAddedCount} out of ${trumpFeed?.items.length || 0}`);

    // Sort by date (newest first)
    allNews.sort((a, b) => b.pubDate - a.pubDate);

    console.log(`📊 Total valid articles after filtering: ${allNews.length}`);

    // Show top 10 most recent articles
    const newsToShow = allNews.slice(0, 10);

    console.log(`\n🎯 TOP 10 ARTICLES TO DISPLAY:`);
    ...
    console.log(`✅ Loaded ${newsToShow.length} news headlines from ${responses.length} sources (${allNews.length} total available)`);

    // Track successful update
    lastNewsUpdate = Date.now();
    newsUpdateCount++;
    console.log(`📊 News update #${newsUpdateCount} completed at ${new Date().toLocaleTimeString()}`);
  }catch(e){
...
function applySettings(){
  ...
  document.getElementById('themeDarkCheck').textContent = settings.theme === 'dark' ? '✓ Active' : '';
  document.getElementById('themeLightCheck').textContent = settings.theme === 'light' ? '✓ Active' : '';
...
setTimeout(async () => {
  ...
  setInterval(() => {
    ...
    if(lastTradeTime > 0 && timeSinceLastTrade > 60000){ // No trades for 1+ minute
      console.warn(`⚠️ No trades received for ${Math.floor(timeSinceLastTrade/1000)}s. 5m: ${win5m.length}, WS state: ${ws?.readyState}`);
    }
    if(win5m.length === 0 && lastTradeTime > 0){
      console.error(`🚨 5m array is empty! Last trade ${Math.floor(timeSinceLastTrade/1000)}s ago. This might cause blank chart.`);
    }
